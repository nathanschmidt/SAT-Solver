...
(* Doesn't hold :( )*)
(* Lemma valuation_eq : forall (x : id) (b : bool) (V V' : valuation), 
  (x !-> b ; V) = (x !-> b ; V') -> V = V'. *)
...
Proof.
  ...
  - simpl. constructor.
    ...
    + constructor.
  - simpl. constructor.
  - simpl. apply ids_union_no_duplicates; assumption.
  - simpl. apply ids_union_no_duplicates; assumption.
  - simpl. apply ids_union_no_duplicates; assumption.
  - simpl. exact IHp.
  Qed.
...
(* Lemma testing : forall (l : list valuation) (x : id) (b : bool),
  NoDup l -> NoDup (map (fun v => x !-> b; v) l).
Proof.
  induction l as [| hd tl IHtl]; intros x b H; simpl.
  - assumption.
  - inversion H. subst. eapply IHtl in H3. constructor.
    + intros contra. rewrite in_map_iff in contra. destruct contra as [v contra].
...
Lemma app_no_dup : forall (A : Type) (l1 l2 : list A) (x : A),
  NoDup l1 -> NoDup l2 -> (In x l1 -> ~ In x l2) -> (In x l2 -> ~ In x l1) ->
  NoDup (l1 ++ l2).
Proof.
  intros A l1. induction l1 as [| hd tl IHtl];
  intros l2 x Hl1 Hl2 Hxl1 Hxl2; simpl.
  - exact Hl2.
  - constructor.
...
Proof.
  induction l as [| hd tl IHtl]; intros acc Hl Hacc Hvx.
  - simpl. exact Hacc.
  - apply IHtl.
...
(* Lemma NO_NAME_YET_LEMMA : forall (v : valuation) (x : id) (p : form),
  exists (v' : valuation), var_in_form x p = true -> 
  v x = v' x /\ In v' (collect_valuations (collect_vars p) [empty_valuation]).
Proof.
  intros v x p. 
  exists (fun y => if var_in_form y p then v y else false).
  intros H. split.
  - rewrite H. reflexivity.
  - unfold var_in_form in *. induction p.
...
Proof.
  ...
  - rewrite eqb_id_refl. reflexivity.
  - reflexivity.
  - rewrite IHp1. rewrite IHp2. do 2 f_equal; 
    apply functional_extensionality; intros x.
    +
  Admitted.

Lemma ANOTHER_NO_NAME_LEMMA : forall (p : form) (v v' : valuation) (vs : list valuation),
  interp v p = true ->
...
Lemma check_valuations_some_interp_true : forall (p : form) (v : valuation) (vs : list valuation),
  check_valuations p vs = Some v -> interp v p = true.
Proof.
  intros p v vs H. induction vs as [| v' vs IHvs].
  - discriminate H.
  - simpl in H. destruct (interp v' p) eqn:Ein.
    + injection H. intros H'. subst. exact Ein.
    + apply IHvs. exact H.
  Qed.
